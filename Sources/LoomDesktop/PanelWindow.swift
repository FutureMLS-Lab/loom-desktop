...
final class PanelWindow: NSPanel, NSWindowDelegate {
    ...
    /// the width the user wants. The height is refused: it belongs to the
    /// content.
    ///
    /// It was briefly the user's too — a drag fixed it, and the rows scrolled
    /// within — but the scroll view had to go (it collapsed the card to a
    /// sliver), and without one a fixed height can only slice pills off
